...
class OperatorConfig(BaseModel):
    ...
    upstream_dependencies: List[str] = Field(
        default_factory=list, description="上游依赖算子ID"
    )
    ...
    env_config: Union[
        ...
        PixiVirtualEnvConfig,
        CondaVirtualEnvConfig,
    ] = Field(..., description="虚拟环境配置（UV/Pixi/Conda三选一）")
    ...
    # 资源需求（可选，用于调度参考）
    resource_request: Optional[Dict[str, float]] = Field(
        default=None, description="资源需求声明"
    )

    other_config: Optional[Dict] = Field(default=None, description="其他配置")
    ...
    def decode_env_vars(cls, v):
        ...
        if isinstance(v, dict):
            return {
                k: Base64Codec.decode(val) if isinstance(val, str) else val
